...
import { otaDeployAuth } from '@/middlewares/otaDeployAuth.middleware';
import { otaPublicHandler, otaAdminHandler, otaStoragePlugin } from '@/services/otaServer.service';
...
// Bundle management (/bundles*) + storage upload/delete/readText/getDownloadUrl
// proxy, used by the CLI's `standaloneRepository`/`standaloneStorage` adapters
// (radix/hot-updater.config.ts). Never reachable without a valid deploy token.
const adminRouter = Router();
adminRouter.use(otaDeployAuth);
...
// though our own external path (/api/ota/admin/bundles) has no double "/api".
adminRouter.all('/*splat', (req: Request, res: Response, next: NextFunction) => {
  if (req.url.startsWith('/bundles')) {
    req.url = `/api${req.url}`;
  }
  return toNodeHandler(otaAdminHandler)(req, res, next);
});
...
// Registered LAST so it never shadows /admin/* above.
